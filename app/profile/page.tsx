            ...
            {user ? (
              <div>
                ...
                {!username && !syncingUI && (
                  <button
                    type="button"
                    onClick={() => { setUsernameDraft(""); setUsernameEditOpen(true); }}
                    className="mt-3 flex w-full items-center gap-2 rounded-xl bg-amber-50 px-3 py-2.5 text-left text-xs font-semibold text-amber-700 hover:bg-amber-100 dark:bg-amber-900/20 dark:text-amber-400 dark:hover:bg-amber-900/30"
                  >
                    <span>⚠️</span>
                    <span>ユーザー名が未設定です。タップして設定してください。</span>
                  </button>
                )}
              </div>
            ) : (
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center px-6" onClick={() => setUsernameEditOpen(false)}>
          ...
          <div className="relative w-full max-w-xs rounded-2xl bg-white p-6 shadow-2xl dark:bg-slate-800" onClick={(e) => e.stopPropagation()}>
            ...
            <input
              ...
              maxLength={30}
              autoFocus
              className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-base text-slate-900 outline-none ring-indigo-400 focus:ring-2 dark:border-slate-600 dark:bg-slate-700 dark:text-white"
            />
