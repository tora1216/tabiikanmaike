...
- ステータスでフィルタリング
- 旅の一覧を開始日順に表示
- 旅の追加モーダルの「インポート」タブから共有IDを入力して旅程をインポート
...
- 旅程を**共有ID**・**QRコード**で共有（Firebase Firestore 管理）
- 共有IDを知っている人のみアクセス可能
- 共有に**合言葉を設定**可能（オーナーのみ設定・変更可）
- 共有モーダルでインポートしたメンバーの名前を確認可能（ログインユーザーのみ記録）
- インポートした旅程を**リアルタイムで共同編集**（どちらの変更も即時反映）
- 旅程を削除すると共有データも自動で無効化（作成者のみ。友人側は Firestore を削除しない）
- 友人がインポートした旅程を削除しても元の共有データには影響なし
